...
Generate person-level and person-period-level datasets. For both, calculate total clicks and total engagement per person or per person-period
```{r}
  person<-daily %>% ungroup() %>% group_by(Id) %>%
                                  mutate(personClick=sum(countClick),
                                         personEngage=sum(countEngage)) %>% 
                                  select(Id,ever_engage,personClick,personEngagemeanDailyClick,meanDailyEngage,
                                         meanDailyMVPA,medianDailyMVPA) %>% 
                                  distinct
  personPeriod<-daily %>% ungroup() %>% group_by(Id,intervention) %>% 
                          mutate(periodClick=sum(countClick),
                                 periodEngage=sum(countEngage)) %>% 
                          select(Id,ever_engage,intervention,personClick,personEngage,meanDailyClickPeriod,meanDailyEngagePeriod,
                                 meanDailyMVPAPeriod,medianDailyMVPAPeriod) %>% distinct
```
